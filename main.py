...
# FIZ isso abaixo no site https://www.asciiart.eu/text-to-ascii-art
...
def correlacao():
    ...
    # QUEM É O M: É A INCLINAÇÃO DA RETA (COEFIIENTE ANGULAR) -> DIZ O QUÃO RAPIDO Y CRESCE/DECRESCE
    # QUEM É O B: ONDE A RETA CRUZA COM O EIXO Y (COEFICIENTE LINEAR) -> O VALOR DE Y QUANDO O X = 0 
    ...
    return render_template_string('''
        <html>
            ...
            <body>
                <div class='container'>
                    <h1> Correlação entre SELIC e Inadimplência
...
def insights3d():
    ...
    trend_color = ['subiu' if x > 0 else 'caiu' if x < 0 else 'estavel' for x in merged['tend_inad']] # Outra forma de fazer o IF e o FOR, você muda a ordem e le a frase de forma mais natural a linguagem normal
    
    # Derivadas discretas
    # Armazena o valor númerico da variação mensal de inadimplencia e selic
    merged['var_inad'] = merged['inadimplencia'].diff().fillna(0)
    merged['var_selic'] = merged['selic_diaria'].diff().fillna(0)
    
    # Isola as colunas de dados economicos qie serão utilizadas na clusterização
    features = merged[['selic_diaria','inadimplencia']].copy()
    
    # Instanciamos o objeto do standart scaler para normalizar a escala
    scaler = StandardScaler()
    
    # Ajusta o scaler e transforma as features para terem a media 0 e desvio padrao 1
    scaled_features = scaler.fit_transform(features)
    
    # Configuramos o KMeans para agrupar os dados em 3 grupos, com semente fixa em 10 inicial
    kmeans = KMeans(n_clusters=3, random_state=42,n_init=10)
    
    # Executa o codigo do algoritimo que já criamos e guarda o rotuno dos grupos (0, 1 ou 2) no dataframe principal
    ...
    # Concatena uma coluna 1s à matriz para calcular o coeficiente linear (interceptor)
    ...
    coeffs, _, _, _ = np.linalg.lstsq(a, y, rcond=None) # z = a*x+b*y+c -> O que seria se fosse na calc
    
    # Criar 30 pontos linearmente espaçados entre o primeiro e o último mês do historico
    ...
    # Calcula valores teoricos de Z (inadimplencia) para cada ponto do grid usando coeficientes.
